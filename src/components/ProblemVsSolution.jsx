...
import { XCircle, CheckCircle2, Clock, Zap, MessageSquare, AlertTriangle, ShieldCheck } from 'lucide-react';

export default function ProblemVsSolution() {
  const painPoints = [
    {
      traditional: 'Endless 60-minute Zoom kickoffs & status meetings wasting founder bandwidth.',
      stackcuts: '100% Asynchronous workflow—submit brief in 2 minutes, get final assets.'
    },
    {
      traditional: 'Slow 2-week turnarounds causing Meta ad account fatigue & rising CPLs.',
      stackcuts: '48-Hour fixed turnaround time for all ad creative growth packs.'
    },
    {
      traditional: 'Boring stock graphics with zero direct-response psychology or visual hook.',
      stackcuts: 'High-contrast split-grids & multi-frame carousels built for thumb-stop CTR.'
    },
    {
      traditional: 'High retainers ($4,000+/mo) with zero revision speed or asset guarantees.',
      stackcuts: 'Frictionless $495 trial packs with 2 free rounds of 48H visual revisions.'
    }
  ];
    ...
    <section id="why-us" className="py-24 bg-[#0B0F17] relative border-t border-[#2A3447]/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-[#161C27]/40 border border-red-500/30 rounded-2xl p-6 sm:p-8 space-y-6 backdrop-blur-md relative overflow-hidden">
            ...
            <div className="space-y-4">
              {painPoints.map((item, idx) => (
                <div key={idx} className="flex items-start gap-3.5 p-4 rounded-xl bg-[#0B0F17]/60 border border-[#2A3447]/80">
                  ...
                  <p className="text-sm text-slate-300 leading-relaxed">{item.traditional}</p>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#161C27] border border-[#00E599]/50 rounded-2xl p-6 sm:p-8 space-y-6 shadow-mint-glow relative overflow-hidden">
            <div className="flex items-center justify-between pb-6 border-b border-[#00E599]/30">
              <div>
                ...
                <h3 className="text-2xl font-display font-bold text-white">100% Asynchronous System</h3>
              </div>
            ...
            <div className="space-y-4">
              {painPoints.map((item, idx) => (
                <div key={idx} className="flex items-start gap-3.5 p-4 rounded-xl bg-[#0B0F17] border border-[#00E599]/30">
                  ...
                  <p className="text-sm text-white font-medium leading-relaxed">{item.stackcuts}</p>
                </div>
              ))}
            </div>

            <div className="pt-4 border-t border-[#00E599]/30 text-center text-xs font-mono text-[#00E599] font-bold">
              ⚡ Result: Lower CPL, Higher CTR, Scale Ads in 48 Hours
            </div>
          </div>
